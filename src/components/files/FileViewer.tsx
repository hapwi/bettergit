...
export const FileViewer = forwardRef<FileViewerHandle, { isActive?: boolean }>(function FileViewer({ isActive }, ref) {
  ...
  const closeOtherFiles = useCallback(
    (keepPath: string) => {
      ...
      const hasDirty = others.some((f) => f.content !== f.originalContent)
      if (hasDirty) {
    ...
    (deletedPath: string) => {
      setOpenFiles((prev) => {
        ...
        const next = prev.filter(
          (f) => f.path !== deletedPath && !f.path.startsWith(deletedPath + "/"),
        )
  ...
  const saveFile = useCallback(async () => {
    if (!repoCwd || !activeFile || activeFile.isBinary) return
    setSaving(true)
    try {
      await writeFile(repoCwd, activeFile.path, activeFile.content)
      setOpenFiles((prev) =>
        prev.map((f) =>
